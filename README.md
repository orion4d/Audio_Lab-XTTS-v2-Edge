...
#### 🖥️ Sinon, via la ligne de commande
...
- `models/`  
  → Contient le modèle XTTS v2 (environ 2–3 Go).

- `voices/`  
  → Déposez ici vos fichiers `.wav` ou `.mp3` pour le clonage de voix.

- `output/`  
  → Tous les fichiers audio générés sont sauvegardés ici avec un horodatage.

---

## 📦 Installation Manuelle (Linux / Mac / Expert)

Si vous n'utilisez pas le script `install.bat`, voici les commandes :
...
- **Premier lancement**  
  Le premier démarrage peut prendre quelques minutes car XTTS doit télécharger les modèles pré-entraînés.

- **Licence Coqui**  
  XTTS v2 est sous licence *Coqui Public Model License* (utilisation non-commerciale par défaut — vérifiez les conditions sur Hugging Face).

- **Connexion Internet**  
  Edge TTS nécessite une connexion active.
